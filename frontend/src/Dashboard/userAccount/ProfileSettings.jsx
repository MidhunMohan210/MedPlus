...
function ProfileSettings({user,refetch }) {
  ...
  useEffect(()=>{
    ...
    // eslint-disable-next-line react/prop-types
    setFormData({name:user.name,email:user.email,gender:user.gender,bloodType:user.bloodType,photo:user.photo,number:user.number})
...
<section className="h-[250px]  flex flex-col mt-[-20] md:flex-row justify-start space-y-10 md:space-y-0 md:space-x-16  my-2 mx-5 md:mx-0 md:my-0 lg:mt-[-20px] mb-[6.5rem]">
      ...
      <div className="max-w-md md:w-[100%] flex flex-col items-start">
        <div className="my-5 items-center before:mt-0.5 before:flex-1 before:border-t before:border-neutral-300 after:mt-0.5 after:flex-1 after:border-t after:border-neutral-300">
